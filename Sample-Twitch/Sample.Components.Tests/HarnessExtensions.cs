...
public static class HarnessExtensions
{
    ...
    public static async Task AssertNotPublished<T>(this ITestHarness harness, string because)
        where T : class =>
        (await harness.Published.Any<T>(TestContext.Current.CancellationToken))
        .Should().BeFalse(because);

    public static async Task AssertSent<T>(this ITestHarness harness, string because)
        where T : class =>
        (await harness.Sent.Any<T>(TestContext.Current.CancellationToken))
        .Should().BeTrue(because);

    public static async Task AssertNotSent<T>(this ITestHarness harness, string because)
        where T : class =>
        (await harness.Sent.Any<T>(TestContext.Current.CancellationToken))
        .Should().BeFalse(because);
    ...
    public static async Task AssertState<TStateMachine, TInstance>(
        this ISagaStateMachineTestHarness<TStateMachine, TInstance> sagaHarness,
        Expression<Func<TInstance, bool>> filter,
        Func<TStateMachine, State> stateSelector,
        string because)
        where TStateMachine : SagaStateMachine<TInstance>
        where TInstance : class, SagaStateMachineInstance
    {
        var existsId = await sagaHarness.Exists(filter, stateSelector);
        existsId.Should().NotBeEmpty(because);
    }

    // Use this only to observe the saga creation stream. For assertions about persisted saga state
    // or copied message values, prefer AssertState with a predicate so the live saga instance is queried.
