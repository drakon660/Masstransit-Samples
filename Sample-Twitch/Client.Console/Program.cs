        ...
        static async Task<string> Execute(OrderModel order)
        {
            try
            {
                ...
                if (responseMessage.StatusCode == HttpStatusCode.Accepted)
                {
                    ...
                    var patchResponse = await _client.PatchAsync($"http://localhost:5000/order/{order.Id:D}/accept", data);

                    patchResponse.EnsureSuccessStatusCode();

                    var patchResult = await patchResponse.Content.ReadAsStringAsync();

                    var statusAddress = $"http://localhost:5000/order/{order.Id:D}/status";

                    do
                    {
                        await Task.Delay(5000);

                        var getResponse = await _client.GetAsync(statusAddress);
